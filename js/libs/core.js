...
var Core = function(){
   ...
   */
  var extend = function(name,func,removeFunc,loadMultiple){
    name = name || '';
    func = func || function(){};

    if(typeof removeFunc === 'boolean'){
      loadMultiple = removeFunc;
      removeFunc = function(){};
    }

    removeFunc = removeFunc || function(){};
    loadMultiple = loadMultiple || false;
  ...
  var load = function(name,params,sel){
    ...
    if(typeof extensions[name]  !== 'undefined'){
      if(extensions[name].loaded === false || extensions[name].loadMultiple === true){
        if(sel){
